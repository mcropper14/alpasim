...
#add to get the output traces 
import json
import time
from pathlib import Path
from threading import Lock
from alpasim_driver.safety_validator import SafetyValidator
import math
...
#added for live trace writing

class StepTraceWriter:
    """Append one JSON object per line for live safety-enforcer consumption."""
    ...
    def __init__(self, output_dir: str | None) -> None:
        ...
        trace_dir = Path(output_dir) / "live_traces"
        trace_dir.mkdir(parents=True, exist_ok=True)
        self._path = trace_dir / "driver_steps.jsonl"
...
class EgoDriverService(EgodriverServiceServicer):
    ...
    def __init__(
        ...
        # Integrate our safety enforcer 
        self._safety_validator = SafetyValidator()
        logger.info("Safety validator enabled")

        # Live per-step reasoning/action trace for external safety enforcer
    ...
    def _trajectory_to_xy_list(self, traj: Trajectory) -> list[list[float]]:
        """Convert trajectory to plain [[x, y], ...] for logging."""
        pts: list[list[float]] = []
        for pose_at_time in traj.poses:
            pts.append([
                float(pose_at_time.pose.vec.x),
                float(pose_at_time.pose.vec.y),
            ])
        return pts

    # def _extract_yaw_from_pose(self, pose: PoseAtTime) -> float:
    #     """Approximate yaw from quaternion. Assumes z-axis yaw is dominant."""
    #     q = pose.pose.rot
    #     siny_cosp = 2.0 * (q.qw * q.qz + q.qx * q.qy)
    #     cosy_cosp = 1.0 - 2.0 * (q.qy * q.qy + q.qz * q.qz)
    #     return float(math.atan2(siny_cosp, cosy_cosp))

    def _extract_yaw_from_pose(self, pose_at_time: PoseAtTime) -> float:
        return float(_quat_to_yaw(pose_at_time.pose.quat))

    def _build_validator_ego_state(
        self,
        pose_snapshot: PoseAtTime,
        session: Session,
    ) -> dict[str, float]:
        speed, _ = self._get_speed_and_acceleration(session)
        return {
            "x": float(pose_snapshot.pose.vec.x),
            "y": float(pose_snapshot.pose.vec.y),
            "speed": float(speed),
            "heading": self._extract_yaw_from_pose(pose_snapshot),
        }

    #furture directory of picking optimal trajectory would be here

    def _estimate_proposed_action_from_trajectory(
        self,
        traj: Trajectory,
        current_speed: float,
    ) -> dict[str, float]:
        pts = [
            (float(p.pose.vec.x), float(p.pose.vec.y))
            for p in traj.poses[:3]
        ]

        if len(pts) < 2:
            return {
                "target_speed": max(float(current_speed), 0.0),
                "steering_angle": 0.0,
            }

        dx1 = pts[1][0] - pts[0][0]
        dy1 = pts[1][1] - pts[0][1]
        heading0 = math.atan2(dy1, dx1)

        steering_angle = 0.0
        if len(pts) >= 3:
            dx2 = pts[2][0] - pts[1][0]
            dy2 = pts[2][1] - pts[1][1]
            heading1 = math.atan2(dy2, dx2)
            steering_angle = heading1 - heading0
    ...
    def _get_live_obstacles_for_validator(self, session: Session) -> list[dict[str, float | int | str]]:
        #integrate lidar in here somehow - for now we return an empty list as a placeholder
        #Talk with the homie Jun Wei 
    ...
    async def drive(
        ...
        speed, acceleration = self._get_speed_and_acceleration(session)

        ego_state = self._build_validator_ego_state(
            pose_snapshot=job.pose,
            session=session,
        )

        proposed_action = self._estimate_proposed_action_from_trajectory(
            traj=alpasim_traj,
            current_speed=speed,
        )
        ...
        obstacles = self._get_live_obstacles_for_validator(session)

        validation_result = self._safety_validator.validate(
            ego_state,
            proposed_action,
            obstacles,
        )
        ...
        trace_record = {
            ...
            "scene_id": session.debug_scene_id,
            "step_time_us": request.time_now_us,
            ...
            "trajectory_xy": self._trajectory_to_xy_list(alpasim_traj),
        }

        try:
            self._trace_writer.write(trace_record)
        except Exception:
            logger.exception("Failed to write live step trace")
        ...
        logger.info(
            "Safety validation: session=%s time=%s is_safe=%s violations=%s validated_action=%s",
            request.session_uuid,
            request.time_now_us,
            validation_result["is_safe"],
            validation_result["violations"],
            validation_result["validated_action"],
        )
