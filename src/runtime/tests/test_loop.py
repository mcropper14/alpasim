...
def test_get_ds_rig_to_aabb_center_transform():
    ...
    qvec = get_ds_rig_to_aabb_center_transform(vehicle_config)
    assert qvec.quat.tolist() == pytest.approx([0.0, 0.0, 0.0, 1.0])  # no rotation
    assert qvec.vec3.tolist() == [0.5, 0.0, 0.5]
